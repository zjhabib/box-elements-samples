...
import { ACCESS_COLLAB, ACCESS_COMPANY, ACCESS_NONE, ACCESS_OPEN, INVITEE_ROLE_EDITOR, PERMISSION_CAN_DOWNLOAD, PERMISSION_CAN_PREVIEW, STATUS_ACCEPTED, TYPE_FOLDER } from '../../../constants';
import { ALLOWED_ACCESS_LEVELS, ANYONE_IN_COMPANY, ANYONE_WITH_LINK, CAN_VIEW_DOWNLOAD, CAN_VIEW_ONLY, COLLAB_GROUP_TYPE, COLLAB_USER_TYPE, PEOPLE_IN_ITEM } from '../constants';
...
export var convertSharedLinkSettings = function convertSharedLinkSettings(newSettings, accessLevel, serverURL) {
...
export var convertCollabsResponse = function convertCollabsResponse(collabsAPIData, avatarURLMap, ownerEmail, isCurrentUserOwner) {
  ...
  var collaborators = entries // Only show accepted collaborations
...
export var convertUserContactsResponse = function convertUserContactsResponse(contactsAPIData, currentUserID) {
  var _contactsAPIData$entr = contactsAPIData.entries,
      entries = _contactsAPIData$entr === void 0 ? [] : _contactsAPIData$entr; // Return all users except for the current user and app users

  return entries.map(function (contact) {
    var id = contact.id,
        email = contact.login,
        name = contact.name,
        type = contact.type;
    ...
    var id = _ref.id,
        email = _ref.email;
    return id !== currentUserID && email && !APP_USERS_DOMAIN_REGEXP.test(email);
  });
};
/**
...
export var convertGroupContactsResponse = function convertGroupContactsResponse(contactsAPIData) {
  var _contactsAPIData$entr2 = contactsAPIData.entries,
      entries = _contactsAPIData$entr2 === void 0 ? [] : _contactsAPIData$entr2; // Only return groups with the correct permissions

  return entries.filter(function (_ref2) {
